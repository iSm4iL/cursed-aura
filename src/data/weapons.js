...
// Собирается обратно в игру через src/game.html: /* @include data/weapons.js */ (см. assemble.js).
  ...
  // Класс НЕ ограничивает пул прокачки на весь забег — любое из 11 базовых заклинаний
  // доступно на левел-апе независимо от класса (collectCandidates больше не фильтрует).
  // Класс даёт: стартовое заклинание + пассивный бонус силы/отката (CLASS_SPELL_DMG_MULT/
  // CLASS_SPELL_CD_MULT в game.html) заклинаниям из своего списка weapons. Группировка —
  // точная спека пользователя, партиция всех 11 заклинаний без пересечений:
  // Вода = волна+кровь+лёд, Огонь = огонь+молния+взрыв, Земля = землетрясение+шипы+лава,
  // Воздух = возд.пуля+барьер (у воздуха своих заклинаний в движке только два).
  // skin — палитра процедурного плейсхолдер-спрайта персонажа на класс (см.
  // makeClassPlayerTexture в game.html); заменяется целиком собственным файлом
  // assets/character_<id>.png по конвенции CUSTOM_ASSETS, если он есть.
  const CLASSES = {
    fire: {
      name: 'Пиромант', color: 0xff7a3d,
      desc: 'Огонь, молния, взрыв — чистый урон на дистанции.',
      weapons: ['fire', 'lightning', 'blast'],
      skin: { cloak: 0x6e2a1a, hood: 0xe0592a, gem: 0xff7a3d, ring: 0xffcf9e }
    },
    water: {
      name: 'Водный маг', color: 0x4fb8e0,
      desc: 'Волна, кровь, лёд — течение, порабощение и контроль.',
      weapons: ['wave', 'charm', 'frost'],
      skin: { cloak: 0x123a52, hood: 0x2f8fc2, gem: 0x4fb8e0, ring: 0xcdeeff }
    },
    earth: {
      name: 'Земляной маг', color: 0xc9a15a,
      desc: 'Землетрясение, металлические шипы, лава — тяжёлый урон по площади.',
      weapons: ['earth', 'metal', 'lava'],
      skin: { cloak: 0x3a2f1f, hood: 0xc9a15a, gem: 0xd9b872, ring: 0xf0dfb0 }
    },
    air: {
      name: 'Ветряной маг', color: 0xbfe9ff,
      desc: 'Воздушная пуля и барьер — скорость и защита.',
      weapons: ['airbullet', 'barrier'],
